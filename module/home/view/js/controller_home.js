function carousel_Brands() {
  ajaxPromise(friendlyURL("?module=home&op=Carrousel_Brand"), "POST", "JSON")
    .then(function (data) {
      for (row in data) {
        $("<div></div>")
          .attr("class", "carousel__elements click_marca")
          .attr("id", data[row].descripcion)
          .appendTo(".carousel__list")
          .html(
            "<img class='carousel__img' id='' src='" +
              data[row].img_marca +
              "' alt='' >"
          );
      }
    ...
    .catch(function () {
      window.location.href =
        "index.php?module=ctrl_exceptions&op=503&type=503&lugar=Carrusel_Brands HOME";
    });
}
...
function loadCategories() {
  ajaxPromise(friendlyURL("?module=home&op=type"), "POST", "JSON")
    .then(function (data) {
      for (row in data) {
        $("<div></div>")
          .attr("class", "card click_cat")
          .attr({ id: data[row].cod_categoria })
...
function loadCatTypes() {
  ajaxPromise(friendlyURL("?module=home&op=cattype"), "POST", "JSON")
    .then(function (data) {
      for (row in data) {
        $("<div></div>")
          .attr("class", "card click_comb")
          .attr({ id: data[row].cod_combustible })
          .appendTo("#containerTypecar")
          .html(
            "<div class='card_image'>" +
              "<img src=" +
              data[row].img_comb +
    ...
    .catch(function () {
      window.location.href =
        "index.php?module=ctrl_exceptions&op=503&type=503&lugar=Types_car HOME";
    });
}

function loadCarroceria() {
  ajaxPromise(friendlyURL("?module=home&op=carroceria"), "POST", "JSON")
    .then(function (data) {
      for (row in data) {
        $("<div></div>")
          .attr("class", "card click_carro ")
          .attr({ id: data[row].descripcion })
          .appendTo("#containerTypecarroceria")
          .html(
            "<div class='card_image '>" +
              "<img src=" +
              data[row].img_carroceria +
              " />" +
              "</div>" +
              "<br>" +
              "<div class='card_title title-black'>" +
...
function loadvisitas() {
  ajaxPromise(friendlyURL("?module=home&op=visit"), "POST", "JSON")
    .then(function (data) {
      for (row in data) {
        $("<div></div>")
          .attr("class", "card click_visitas ")
          .attr({ id: data[row].cod_modelo })
          .appendTo("#containervisitas")
          .html(
            "<div class='card_image '>" +
              "<img src=" +
              data[row].img_car +
...
function clicks() {
  ...
  localStorage.removeItem("filter_pag");
  ...
  $(document).on("click", ".click_marca", function () {
    ...
    setTimeout(function () {
      window.location.href = friendlyURL("index.php?module=shop");
    }, 500);
  });
  ...
  $(document).on("click", "#btn-more-books", function () {
    ...
    ajaxPromise(
      ...
      .then(function (data) {
        if (limit === 9) {
          $(
            '<button class="no-results" id="">No hay mas libros disponibles....</button></br>'
          ).appendTo(".btn-more-books");
        } else {
          $(
            '<button class="load_more_button" id="load_more_books">LOAD MORE</button>'
          ).appendTo(".btn-more-books");
        }

        for (i = 0; i < limit; i++) {
          $("<div></div>")
            .attr({ id: "books_car", class: "books_car" })
            .appendTo(".containerbooks")
            .html(
              '<div  style="border: 1px solid black; display: flex; align-items: auto;margin-left:30%">' +
                ...
                '" +  alt="example image" style="flex: 1; margin-right: 1rem;" height: 50vw;>' +
                '<div style="flex: 1;">' +
                ...
                ' target="_blank" class="btn btn-danger btn-block btn-lg adjust-border-radius">MORE INFO</a>' +
                "</div>" +
                "</div>" +
                "<br>"
            );
