function load_marcas() {
    ...
    ajaxPromise(friendlyURL('?module=search&op=search_marca'), 'POST', 'JSON')
        .then(function (data) {
...
function load_categoria(marca) {
    ...
    if (marca === undefined) {
        ajaxPromise(friendlyURL('?module=search&op=search_categoria_null'), 'POST', 'JSON')
            .then(function (data) {
        ...
        ajaxPromise(friendlyURL('?module=search&op=search_categoria'), 'POST', 'JSON', {'marca': marca})
            .then(function (data) {
                $('<option>Categoria</option>').attr('selected', true).attr('disabled', true).appendTo('.search_categoria')
                for (row in data) {
                    $('<option value="' + data[row].cod_categoria + '">' + data[row].nombre_cat + '</option>').appendTo('.search_categoria')
