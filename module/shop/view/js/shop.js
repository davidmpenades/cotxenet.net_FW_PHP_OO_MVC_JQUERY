function ajaxForSearch(url, filter, total_prod, items) {
  ajaxPromise(friendlyURL(url), "POST", "JSON", { 'filter': filter, 'total_prod': total_prod,'items':items })
    .then(function (data) {
...
function loadListCar(total_prod=0,items=3) {
  ...
  if (filtro != false) {
    ajaxForSearch("?module=shop&op=filter", filtro,total_prod,items);
    localStorage.setItem('filter_pag',JSON.stringify(filtro));
  } else if (home_carro != false) {
        
    ajaxForSearch("?module=shop&op=filter_carro", home_carro,total_prod,items);
    localStorage.setItem('filter_pag',JSON.stringify(home_carro));
    remove(filtro);
  }
  else if (home_tipo != false) {
     
    ajaxForSearch("?module=shop&op=filter_tipo", home_tipo,total_prod,items);
    ...
    ajaxForSearch("?module=shop&op=filter_marca", home_marca,total_prod,items);
    localStorage.setItem('filter_pag',JSON.stringify(home_marca));
    remove(filtro);
  }
  else if (home_comb != false) {
    ajaxForSearch("?module=shop&op=filter_comb", home_comb,total_prod,items);
    localStorage.setItem('filter_pag',JSON.stringify(home_comb));
    remove(filtro);
  }
  else if (search != false) {
    
    ajaxForSearch("?module=shop&op=filter", search,total_prod,items);
    ...
    ajaxForSearch("?module=shop&op=filter", homevisitas,total_prod,items);
    ...
    ajaxForSearch("?module=shop&op=all_cars",undefined,total_prod,items);
...
function loadDetails(id_car) {
  console.log(id_car);
  ajaxPromise("?module=shop&op=one_car" ,"POST","JSON", {'id':id_car})
    .then(function (data) {
      console.log(data);
      $("#all_cars").empty();
      ...
      for (row in data) {
        $("<div></div>")
          .attr({ id: data[0].id_car, class: "date_img_dentro" })
          .appendTo(".date_img")
          .html(
            "<br>" +
...
function more_cars_related(marca) {
    let items = 0;
    ajaxPromise(friendlyURL('?module=shop&op=count_cars_related'), 'POST', 'JSON', { 'marca': marca })
      .then(function (data) {
...
function cars_related(items = 0, marca, total_items) {
    ...
    ajaxPromise("?module=shop&op=cars_related", 'POST', 'JSON', { 'marca': marca, 'items': items })
      .then(function (data) {
        if(data != 'error'){
...
function filter_button() {
  ...
  $(document).on("click", ".filter_button", function () {
    ...
    if (filter) {
      ...
      ajaxForSearch("?module=shop&op=all_cars");
    }
  });
}
function remove() {
